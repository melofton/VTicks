...
```{r}
time_length <- rep(NA,nsites )

time_length[1] <- 17 #number of available data points at each site
time_length[2] <- 100

y <- array(NA, dim = c(nsites, max(time_length)))

y[ , 1]

for(j in 1:nsites){
  for(i in 2:time_length[j]){
  
    x[j, i] <- x[j, i- 1]
    y[j, i] <- dnorm(x[j, i], sd = sigma)
  }
}
